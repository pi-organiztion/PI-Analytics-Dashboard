...
from modules import preprocessing, wc_analytics, driver_analytics, layouts,wc_data
import json
import os
...
app.layout = layouts.create_app_layout_reload
server = app.server
...
def show_wc_task_bars(split_by_types, wc_range, lookback_period):
  end_date = wc_data.global_data['end_dates'][lookback_period]
  split = True if split_by_types == 'Yes' else False

  if split:
    fig = wc_analytics.create_task_type_bars(wc_data.global_data['tasks'],
                                             wc_range,
                                             end_date,
                                             lookback_period,
                                             task_type_colors)
  else:
    fig = wc_analytics.create_task_bars(wc_data.global_data['tasks'],
                                        wc_range,
                                        end_date,
                                        lookback_period,
                                        pi_colorscale)
...
def show_wc_queue_bars(rollover_period, wc_range, lookback_period):
  end_date = wc_data.global_data['end_dates'][lookback_period]
  rollover_period = wc_data.global_data['rollover_periods'][rollover_period] if rollover_period != 'No' else False

  if rollover_period:
    fig = wc_analytics.create_rollover_queue_bars(wc_data.global_data['tasks'],
                                                  rollover_period,
                                                  wc_range, 
                                                  end_date,
                                                  lookback_period,
    ...
    fig = wc_analytics.create_queue_bars(wc_data.global_data['tasks'],
                                         wc_range,
                                         end_date,
                                         lookback_period,
                                         pi_colorscale)
...
def show_wc_task_type_pi(lookback_period):
  end_date = wc_data.global_data['end_dates'][lookback_period]
  fig = wc_analytics.create_task_type_pi(wc_data.global_data['tasks'],
                                         end_date,
                                         lookback_period,
                                         task_type_colors)
  return fig
...
def show_top_10_tasks(lookback_period):
  end_date = wc_data.global_data['end_dates'][lookback_period]
  options, value = wc_analytics.get_top_10_tasks_dropdown(wc_data.global_data['tasks'], end_date)
...
def show_task_time_distrib(task_info, lookback_period):
  end_date = wc_data.global_data['end_dates'][lookback_period]
  fig = wc_analytics.create_task_time_distrib(wc_data.global_data['tasks'], task_info, end_date)
  return fig
...
def show_wc_efficiency_table(lookback_period):
  end_date = wc_data.global_data['end_dates'][lookback_period]
  table_df = wc_analytics.create_efficiency_table(wc_data.global_data['tasks'], end_date)
...
def show_driver_task_lines(lookback_period):
  end_date = wc_data.global_data['end_dates'][lookback_period]
  fig = driver_analytics.create_task_lines(wc_data.global_data['tasks'],
                                           end_date,
                                           lookback_period,
                                           wc_data.global_data['driver_colors'])
  return fig
...
def show_top_10_tasks(lookback_period):
  end_date = wc_data.global_data['end_dates'][lookback_period]
  options, value = driver_analytics.get_top_10_tasks_dropdown(wc_data.global_data['tasks'], end_date)
  return options, value
...
def show_driver_dur_bars(task_info, lookback_period):
  end_date = wc_data.global_data['end_dates'][lookback_period]
  fig = driver_analytics.create_dur_bars(wc_data.global_data['tasks'],
                                          task_info,
                                          end_date,
                                          lookback_period,
                                          wc_data.global_data['driver_colors'])
...
def show_driver_dist_bars(task_info, lookback_period):
  end_date = wc_data.global_data['end_dates'][lookback_period]
  fig = driver_analytics.create_dur_dist_bars(wc_data.global_data['tasks'],
                                              task_info,
                                              end_date,
                                              lookback_period,
                                              wc_data.global_data['driver_colors'])
...
def show_driver_efficiency_table(lookback_period):
  end_date = wc_data.global_data['end_dates'][lookback_period]
  table_df = driver_analytics.create_efficiency_table(wc_data.global_data['tasks'], end_date)
  return table_df.to_dict('records'), [{'name': i, 'id': i} for i in table_df.columns]
...
def show_driver_task_pi(lookback_period):
  end_date = wc_data.global_data['end_dates'][lookback_period]
  fig = driver_analytics.create_task_pi(wc_data.global_data['tasks'],
                                        end_date,
                                        lookback_period,
                                        wc_data.global_data['driver_colors'])
  return fig
