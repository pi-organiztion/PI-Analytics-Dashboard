...
Functions:
  ...
  create_dist_bars(tasks,
                   end_date,
                   lookback_period,
                   driver_colors):
    Creates a bar graph containing the distance travelled by each driver for the given task.
...
def create_dist_bars(tasks, task_info, end_date, lookback_period, driver_colors):
  """Creates a bar graph containing the distance travelled by each driver for the given task.
  ...
  df = df.groupby('Driver').median()
  df['Times Completed'] = times_completed 
  df = df.reset_index()
  ...
  fig = px.bar(df,
               ...
               color_discrete_map=driver_colors,
               text='Distance',
               title=f'Average Distance for Task {rank} - Completed {sum(df["Times Completed"])} Times - {task_type} ({lookback_period})',
               labels={'Distance': 'Average Distance (meters)'},
               hover_data=['Times Completed'])
  
  hovertemp = ('Driver: %{x} <br>Avg Distance: %{y} m <br>Completed: %{customdata} times<extra></extra>')
  fig.update_traces(hovertemplate=hovertemp, textposition='inside', texttemplate='%{text} m')

  mean_dist = np.mean(df['Distance'])
  avg_line_range = df['Driver'].to_numpy()
  fig.add_trace(go.Scatter(x=[avg_line_range[0], avg_line_range[-1]],
                           y=[mean_dist, mean_dist],
                           cliponaxis=True,
                           mode='lines',                       
                           line=dict(color='limegreen', width=2, dash='solid'),
                           name=f'Average {mean_dist:.1f} m',
                           hovertemplate=f'Average {mean_dist:.1f} m<extra></extra>'))
...
def create_task_pi(tasks,
  ...
  hovertemp = ('Driver: %{customdata[0][0]} <br>Tasks Completed: %{customdata[0][1]} <br>Percentage of Total Tasks: %{customdata[0][2]} ')
